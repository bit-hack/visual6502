...
//     pullup: seg[1]=='+'
//     state    -
//     gates    - list of transistors that share the same gate
//     c1c2s    - list of nodes that have this as a c1 or a c2
// }
var nodes = new Array();
var transistors = {};
var nodenamelist = [];
...
// recalc list contains all nodes that a due to be evaluated
var recalclist = new Array();
// recalc hash is used to avoid adding a node twice to the recalclist
...
// create 'nodes' list from transdefs
function setupTransistors(){
    // setup transistor definitions
    for(i in transdefs){
        // get transistor node
        var tdef = transdefs[i];
        var name = tdef[0];
        var gate = tdef[1]; // gate
        var c1 = tdef[2];   // connection 1
        var c2 = tdef[3];   // connection 2
        var bb = tdef[4];   // bounding box
        if(c1==ngnd) {
            // swap connections so c2 is ngnd
            c1=c2;
            c2=ngnd;
        }
        if(c1==npwr) {
            // swap connections so c2 is npwr
            c1=c2;
            c2=npwr;
        }
        ...
        // add this node to the c1 it touches
        nodes[c1].c1c2s.push(trans);
        // add this node to the c2 it touches
        nodes[c2].c1c2s.push(trans);
        // add to the transistors list
...
// originally to highlight using a list of node numbers but can now include
// transistor names
...
// find 'n' in nodenames list and return its index
function nodeName(n) {
    for(var i in nodenames){
...
function setupChipLayoutGraphics(){
    ...
    setStatus('Ready!'); // would prefer chipStatus but it's not importent
...
// XXX: SIM
// crunch a list of nodes to waiting to be evaluated
function recalcNodeList(list){
    ...
    for(var j=0;j<100;j++){
        // if nothing left to simulate then break
        if(list.length==0)
            return;
//      if(ctrace) {
//          var i;
//          for(i=0;i<traceTheseNodes.length;i++) {
//              if(list.indexOf(traceTheseNodes[i])!=-1)
//                  break;
//          }
//          if((traceTheseNodes.length==0)||
//             (list.indexOf(traceTheseNodes[i])==-1)) {
//              console.log('recalcNodeList iteration: ', j, list.length, 'nodes');
//          } else {
//              console.log('recalcNodeList iteration: ', j, list.length, 'nodes', list);
//          }
//      }
        // call recalcNode for each node waiting to be updated
        list.forEach(recalcNode);
        // new list to update has been generated
        list = recalclist;
        // create blank lists to receive update information
        // note: these lists are global and appended to by addRecalcNode
        recalclist = new Array();
        recalcHash = new Array();
    }
//  if(ctrace){
...
//  }
    group.forEach(
        ...
        function(i){
            var n = nodes[i];
            // if node is not changing state do nothing
            if(n.state == newState)
                return;
            // update node state
            n.state = newState;
            n.gates.forEach(
                // closeure to capture n
                function(t){
                    if(n.state)
                        turnTransistorOn(t);
                    else
...
function addRecalcNode(nn){
       ...
       if(recalcHash[nn] == 1)
           return;
       // add to the recalc node list
       recalclist.push(nn);
       // add to the recalc hash set
       recalcHash[nn] = 1;
}
...
function getNodeGroup(i){
    // create a new group
    group = new Array();
    // add the first node to group and expand
    addNodeToGroup(i);
}

// XXX: SIM
// note: groups could be precomputed apart from the not adding 't' if its LOW
function addNodeToGroup(i){
    // if node is already in group then we are done
    if(group.indexOf(i) != -1)
        return;
    // add node to group
    group.push(i);
    // if node is power of ground we are done
    if(i==ngnd) return;
    if(i==npwr) return;
    // find all nodes whos collectors are attached to this node
    // XXX: is this right?
    nodes[i].c1c2s.forEach(
        function(t){
            // if t is LOW then return
            if(!t.on)
                return;
            var other;
            // add the node that is not this transistor itself
...
// simulate until sync asserts or read write
function goUntilSyncOrWrite(){
    halfStep();
    cycle++;
    while(!isNodeHigh(nodenames['clk0']) ||
...
function step(){
    ...
    // if we have a checksum to compute
    if(goldenChecksum != undefined)
        // checksum is old checksum + first two memory pages
        traceChecksum = adler32(
            traceChecksum+s+m.slice(0,511).toString(16));
    // step half a cycle
    halfStep();
    // update the chip layout rendering
    if(animateChipLayout)
        refresh();
    // increment cycle counter
    cycle++;
    // update chip status diagram
    chipStatus();
}

// simulate a single clock phase with no update to graphics or trace
function halfStep(){
    // get binary clk state
    var clk = isNodeHigh(nodenames['clk0']);
    // evaluate any clock callbacks
...
// read p (flags) register
function readP(){
    return readBits('p', 8);
}

// read flags register as string
...
// read full program counter
function readPC(){
    return (readBits('pch', 8)<<8) + readBits('pcl', 8);
}

// read program counter low
function readPCL(){
    return readBits('pcl', 8);
}

// read program counter high
function readPCH(){
...
function busToString(busname){
    ...
    if(busname=='Phi')
        ...
        return '&Phi;' + (isNodeHigh( nodenames[ 'cp1' ] ) ? '1' : '2');
    if(busname=='Execute')
...
// run for an extended number of cycles, with low overhead, for interactive
// programs or for benchmarking
...
// helper function: allows us to poll 'running' without resetting it when we're
// re-scheduled
function goForN(n){
    ...
    if((headlessSteps<0) && running){
        setTimeout("goForN("+n2+")",0);
        // re-submit ourselves if we are meant
        // to free-run
        return;
    }
    running = false;
...
function estimatedHz(){
    ...
    // convert from phases per millisecond to Hz
    HzEstimate=HzEstimate*1000/2;
    if(HzEstimate < 5) {
        // quicker
        HzSamplingRate = 5;
    }
    ...
    //xxx: wrong way to average speeds
    prevHzEstimate1=(HzEstimate+prevHzEstimate1+prevHzEstimate2)/3;
